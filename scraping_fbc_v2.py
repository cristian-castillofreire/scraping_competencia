...
async def run_scraping(product_id: str, task_id: int):
    ...
    try:
        ...
        # ✅ Cargar login
        await tab_driver.get(f"https://www.falabella.com/falabella-cl/myaccount/login", wait_load=True)
        ...
        # 🟢 Ingresar contraseña
        await send_keys_verificado(driver=tab_driver, by=By.ID, element="password", input_text=ACCOUNT_DATA[task_id]["pw"], element_description="Contraseña")
        ...
        # 🟢 Eliminar producto
        decrement_button = await tab_driver.find_element(By.XPATH, "//button[@data-testid='15643401-new-design-decrement-button']", timeout=10)
        await decrement_button.click(move_to=True)
...
async def main():
    ...
    for chunk in chunks:
        tasks = [run_scraping(pid, i) for i, pid in enumerate(chunk)]
